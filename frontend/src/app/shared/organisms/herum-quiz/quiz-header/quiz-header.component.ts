...
export class QuizHeaderComponent implements OnInit, OnChanges {
  ...
  private calculateMaxQuestionsToShow() {
    const containerWidth = this.el.nativeElement.querySelector(
  ...
  _getQuestionStatus(question: Question, questionIndex: number) {
    ...
    if (this.isQuestionAnswered(question)) return 'answered';

    return 'unanswered';
  }

  _previousViewIndex() {
    if (0 < this.startIndexOfQuestions + this.viewIndex) this.viewIndex--;
  }
  ...
  get endIndexOfQuestions() {
    return Math.max(
      this.quizData.currentQuestion + this.maxQuestionsToShow / 2,
      this.maxQuestionsToShow,
    );
  }

  get slicedQuestionsList() {
    if (!this.isViewIndexNeeded) return this.quizData.questions;
  ...
  private isQuestionAnswered(question: Question) {
    let userAnswerCount = this.quizData.userAnswers?.find(
