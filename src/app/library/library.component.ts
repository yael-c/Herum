...
export class LibraryComponent implements OnInit {
    ...
    ngOnInit(): void {
        forkJoin({
            userData: this.graphqlService.executeQuery<any, any>('', LibraryQueries.GET_CURRENT_USER_SUBSCRIPTIONS, {}),
            collections: this.graphqlService.executeQuery<any, any>('', LibraryQueries.GET_ALL_COLLECTIONS, {})
        })
            ...
            .subscribe({
                next: (results) => {
                    if (results.userData) {
                        ...
                        this.subscribedCollections = this.currentUser?.user?.subscribedCollections?.map((sub: {
                            collectionPreview: any;
                        }) => sub.collectionPreview) || [];
                    }
